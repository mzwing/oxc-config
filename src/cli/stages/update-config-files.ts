...
const legacyConfigName = /eslint|prettier/i
...
export async function updateConfigFiles(result: MigrationOptions): Promise<void> {
  ...
  const lintOptions: [string, unknown][] = result.frameworks
    .filter(framework => framework !== 'astro' && framework !== 'svelte')
    .map(framework => [framework, true])
  if (result.typescript) lintOptions.push(['typescript', true])
  if (lintIgnores.length) lintOptions.push(['ignores', lintIgnores])

  const formatOptions: [string, unknown][] = []
  if (result.frameworks.includes('svelte')) formatOptions.push(['svelte', true])
  if (formatIgnores.length) formatOptions.push(['ignores', formatIgnores])
  ...
  if (result.frameworks.includes('astro'))
    p.note('Astro linting and formatting are not currently supported by Oxc.', 'Astro migration gap')

  const legacyFiles = (await fsp.readdir(cwd)).filter(file => legacyConfigName.test(file)).sort()
